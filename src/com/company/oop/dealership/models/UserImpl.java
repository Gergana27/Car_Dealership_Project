...
public class UserImpl implements User {
    ...
    private static final String USERNAME_PATTERN_ERROR_MESSAGE = "Username contains invalid symbols!";
    private static final String USERNAME_LEN_ERROR_MESSAGE = format("Username must be between %d and %d characters long!",
    ...
    private static final String PASSWORD_PATTERN_ERROR_MESSAGE = "Password contains invalid symbols!";
    private static final String PASSWORD_LEN_ERROR_MESSAGE = format("Password must be between %s and %s characters long!",
            PASSWORD_LEN_MIN, PASSWORD_LEN_MAX);

    public static final int LASTNAME_LEN_MIN = 2;
    public static final int LASTNAME_LEN_MAX = 20;
    private static final String LASTNAME_LEN_ERROR_MESSAGE = format("Lastname must be between %s and %s characters long!",
            LASTNAME_LEN_MIN, LASTNAME_LEN_MAX);
    public static final int FIRSTNAME_LEN_MIN = 2;
    public static final int FIRSTNAME_LEN_MAX = 20;
    private static final String FIRSTNAME_LEN_ERROR_MESSAGE = format("Firstname must be between %s and %s characters long!",
            FIRSTNAME_LEN_MIN, FIRSTNAME_LEN_MAX);
    ...
    private void setUsername(String username) {
        ValidationHelpers.validatePattern(username, USERNAME_REGEX_PATTERN, USERNAME_PATTERN_ERROR_MESSAGE);
        ValidationHelpers.validateIntRange(username.length(), USERNAME_LEN_MIN, USERNAME_LEN_MAX, USERNAME_LEN_ERROR_MESSAGE);
        this.username = username;
    }

    private void setFirstName(String firstName) {
        ValidationHelpers.validateIntRange(firstName.length(), FIRSTNAME_LEN_MIN, FIRSTNAME_LEN_MAX, FIRSTNAME_LEN_ERROR_MESSAGE);
        this.firstName = firstName;
    }

    private void setLastName(String lastName) {
        ValidationHelpers.validateIntRange(lastName.length(), LASTNAME_LEN_MIN, LASTNAME_LEN_MAX, LASTNAME_LEN_ERROR_MESSAGE);
        this.lastName = lastName;
    }

    private void setPassword(String password) {
        ValidationHelpers.validatePattern(password, PASSWORD_REGEX_PATTERN, PASSWORD_PATTERN_ERROR_MESSAGE);
        ValidationHelpers.validateIntRange(password.length(), PASSWORD_LEN_MIN, PASSWORD_LEN_MAX, PASSWORD_LEN_ERROR_MESSAGE);
    ...
    public void addVehicle(Vehicle vehicle) {
        ...
        if (!isAdmin() && !userRole.equals(UserRole.VIP) && vehicles.size() >= NORMAL_ROLE_VEHICLE_LIMIT) {
